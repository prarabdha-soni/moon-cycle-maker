...
import {
  Droplet, Heart, Activity, Moon, Pill, Brain, Coffee, Flame,
  BedDouble, Sparkles, Wind, Scale, Check,
} from "lucide-react";
      ...
      <div className="space-y-6 px-5 pt-2 pb-4">
        ...
        {categories.map((cat) => (
          <section key={cat.title}>
            <div className="mb-2">
              <h2 className="font-display text-[16px] font-semibold text-foreground">{cat.title}</h2>
              ...
              {cat.items.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[11px] text-center leading-tight text-foreground">{label}</span>
                  </button>
                );
              })}
            </div>
